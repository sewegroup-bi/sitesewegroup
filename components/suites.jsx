  ...
  const suites = [
    {
      ...
      body: tx('Um único painel com os KPIs que importam: Suprimentos, Comercial, Financeiro e Operacional, consolidados para o C-Level. Drill-down até o documento na mesma tela.'),
      kpis: [{ v: '7 filiais', l: tx('unificadas em tempo real') }, { v: 'Zero', l: tx('planilhas paralelas na diretoria') }, { v: '< 2 min', l: tx('para consolidar toda a DRE') }],
      bullets: [
        tx('Consolidação multi-filial em uma única tela, sem planilha manual'),
        tx('Simulação de cenários (what-if) para preço e custo logístico'),
        tx('Crescimento YoY, ciclo de caixa e produtividade lado a lado'),
        tx('Alerta automático quando EBITDA ou ruptura fogem do planejado'),
      ],
    ...
    },
    {
      ...
      label: tx('Comercial'),
      icon: 'trending',
      tagline: tx('Cada vendedor, cada cliente, cada SKU.'),
      title: tx('Positivação, churn e cross-sell em tempo real.'),
      ...
      kpis: [{ v: '+30%', l: tx('reativação de inativos') }, { v: '+25%', l: tx('aumento de LTV') }, { v: '+35%', l: tx('produtividade de vendas') }],
      bullets: [
        tx('Positivação MTD por vendedor, rota e filial'),
        tx('Alerta de Curva A com risco de churn'),
        tx('Cross-sell automático por perfil do cliente'),
        tx('Mapa de calor de oportunidades por região'),
      ],
      dashboard: 'comercial',
    },
    {
      key: 'suprimentos',
      label: tx('Suprimentos'),
      icon: 'boxes',
      tagline: tx('Estoque na medida exata.'),
      title: tx('Compre pela demanda real. Zere a ruptura de Curva A.'),
      body: tx('IA preditiva que lê 18 meses de histórico e ajusta pedidos em tempo real. Evita excesso de mercadoria parada e zera a ruptura nos itens que pagam a conta.'),
      kpis: [{ v: '-15%', l: 'custos operacionais' }, { v: '+90%', l: tx('assertividade') }, { v: '-20%', l: tx('estoque parado') }],
      bullets: [
        tx('Alerta de ruptura antes que o item falte na prateleira'),
        tx('Sugestão de compra automática enviada ao ERP'),
        tx('Curva ABC viva com reclassificação contínua'),
        tx('Identifica fornecedores com performance declinante'),
      ],
    ...
    },
    {
      ...
      label: tx('Financeiro'),
      icon: 'dollar',
      tagline: tx('A margem que sobra, por SKU.'),
      title: tx('DRE automatizado, margem líquida real, fluxo de caixa vivo.'),
      body: tx('Descontando rebate, frete, verba, imposto e devolução. Closing de competência em 2 dias, com rastreabilidade até o documento fiscal de origem.'),
      kpis: [{ v: '+1,1pp', l: tx('margem líquida') }, { v: '100%', l: tx('auditável') }, { v: '2 dias', l: tx('até o closing') }],
      bullets: [
        tx('DRE gerencial automatizado por filial e centro de custo'),
        tx('Margem líquida real por SKU, cliente e rota'),
        tx('Previsão de inadimplência por padrão de pagamento'),
        tx('Conciliação automática de rebate e verba'),
      ],
...
function SuiteDashboard({ kind }) {
  if (kind === 'suprimentos') {
    return (
      <QlikFrame title={tx('SUPRIMENTOS · CURVA ABC · RUPTURA')} subtitle={tx('MTD · 7 filiais')} tabs={[tx('Visão geral'), tx('Ruptura'), 'Fornecedores', 'Giro']} activeTab={1}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 10 }}>
          <QlikKPI compact label={tx('Ruptura Curva A')} value="2,1%" delta="-0,8pp" trend="up" color={Q.pos} spark={<Sparkline data={[3.5,3.2,2.9,2.7,2.4,2.3,2.1]} color={Q.pos} fill="rgba(46,139,87,0.15)"/>}/>
          <QlikKPI compact label={tx('SKUs Ativos')} value="3.482" delta="+12" trend="up" color={Q.navy}/>
          <QlikKPI compact label={tx('Giro Médio')} value="4,8x" delta="+0,6" trend="up" color={Q.navy}/>
          <QlikKPI compact label={tx('Estoque Parado')} value="R$ 1,2M" delta="-20%" trend="up" color={Q.pos}/>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 10 }}>
          <QlikHBars
            title={tx('Top 8 SKUs com risco de ruptura · próx. 7 dias')}
            rows={[
              { label: tx('Ração Premier 15kg'),  v: 92, display: '92%', color: '#c53030' },
              { label: tx('Shampoo Pet 5L'),      v: 84, display: '84%', color: '#c53030' },
              { label: tx('Coleira Antipulgas'),  v: 72, display: '72%', color: '#c27a00' },
              { label: tx('Vacina Pol. V8'),      v: 64, display: '64%', color: '#c27a00' },
              { label: tx('Ração Felina Adult'),  v: 48, display: '48%', color: '#c27a00' },
              { label: tx('Cama Grande'),         v: 35, display: '35%' },
              { label: tx('Brinquedo Mordedor'),  v: 22, display: '22%' },
              { label: tx('Areia Sanitária 4kg'), v: 14, display: '14%' },
            ]}
            max={100}
          />
          <div style={{ display: 'grid', gridTemplateRows: 'auto 1fr', gap: 10 }}>
            ...
            <QlikArea
              title={tx('Previsão de demanda próx. 30 dias')}
              subtitle={tx('IA · assertividade 91,4%')}
  ...
  if (kind === 'comercial') {
    return (
      <QlikFrame title={tx('COMERCIAL · POSITIVAÇÃO · CHURN')} subtitle={tx('MTD · Todos os vendedores')} tabs={[tx('Visão geral'), tx('Positivação'), 'Churn', tx('Cross-sell')]} activeTab={2}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 10 }}>
          <QlikKPI compact label={tx('Positivação MTD')} value="84,2%" delta="+3,2pp" trend="up" color={Q.navy}/>
          <QlikKPI compact label={tx('Clientes Ativos')} value="1.248" delta="+38" trend="up" color={Q.navy}/>
          <QlikKPI compact label={tx('Em risco · Curva A')} value="17" delta="+3" trend="down" color={Q.neg}/>
          <QlikKPI compact label={tx('LTV médio')} value="R$ 128k" delta="+25%" trend="up" color={Q.navy}/>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
          <QlikArea title={tx('Positivação diária')} subtitle={tx('MTD · Meta 80%')} data={[72,74,73,76,78,77,80,79,82,81,83,84]} labels={['D1','','D5','','D9','','D13','','D17','','D21','']} height={150}/>
          <div style={{ background: '#fff', border: `1px solid ${Q.line}`, borderRadius: 10, padding: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}>
            <QlikDonut value={84} label={tx('Positivação')} sublabel="Meta 80%" color={Q.turq2}/>
            <div style={{ width: 1, alignSelf: 'stretch', background: Q.line }}/>
            <QlikDonut value={92} label={tx('Meta ciclo')} sublabel="R$ 4,7M" color={Q.navy}/>
          </div>
        </div>
        <QlikTable
          title={tx('Carteira em risco de churn · Curva A (top 5)')}
          columns={[
            { key: 'cli', label: 'Cliente' },
            { key: 'ltv', label: 'LTV', align: 'right', mono: true },
            { key: 'dias', label: tx('Dias s/compra'), align: 'right', mono: true },
            { key: 'sug', label: tx('Ação sugerida pela IA') },
          ...
          rows={[
            { cli: tx('Distribuidora Sul'),    ltv: 'R$ 142k', dias: 47, sug: tx('Contato comercial hoje'),       risk: 'Alto' },
            { cli: tx('Agrocenter Sul'),     ltv: 'R$ 98k',  dias: 38, sug: tx('Reativação + cross-sell Curva B'), risk: 'Alto' },
            { cli: tx('Pet House Oeste'),    ltv: 'R$ 76k',  dias: 31, sug: tx('Ligação de relacionamento'),    risk: tx('Médio') },
            { cli: tx('Vetclin Distrib.'),   ltv: 'R$ 64k',  dias: 28, sug: tx('Oferta programa fidelidade'),   risk: tx('Médio') },
            { cli: tx('Multiagro Norte'),    ltv: 'R$ 58k',  dias: 22, sug: tx('Acompanhamento próx. ciclo'),   risk: tx('Médio') },
          ]}
          compact
        />
      </QlikFrame>
  ...
  if (kind === 'financeiro') {
    return (
      <QlikFrame title={tx('FINANCEIRO · DRE · MARGEM LÍQUIDA')} subtitle={tx('Competência · Abril/2026')} tabs={['DRE', tx('Margem por SKU'), tx('Fluxo de Caixa'), 'Rebate']} activeTab={0}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 10 }}>
          <QlikKPI compact label={tx('Receita Bruta')} value="R$ 18,4M" delta="+14%" trend="up" color={Q.navy}/>
          <QlikKPI compact label={tx('Margem Líquida')} value="18,4%" delta="+1,1pp" trend="up" color={Q.navy}/>
          <QlikKPI compact label={tx('Rebate Realizado')} value="R$ 1,2M" delta="+8%" trend="up" color={Q.navy}/>
          <QlikKPI compact label={tx('Inadimplência')} value="2,4%" delta="-0,3pp" trend="up" color={Q.pos}/>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 10 }}>
          <QlikTable
            title={tx('DRE Gerencial · consolidado')}
            columns={[
              { key: 'conta', label: 'Conta' },
              { key: 'val',   label: 'R$',   align: 'right', mono: true },
              { key: 'pct',   label: '% RL', align: 'right', mono: true },
              { key: 'vs',    label: tx('vs Mês Ant.'), align: 'right', mono: true },
            ]}
            rows={[
              { conta: tx('Receita Líquida'),      val: '18.420.410', pct: '100,0%', vs: '+14,1%' },
          ...
          />
          <div style={{ display: 'grid', gridTemplateRows: 'auto 1fr', gap: 10 }}>
            <QlikArea title={tx('Fluxo de caixa · próx. 60 dias')} subtitle={tx('Projeção · IA 94% assert.')} data={[2.1,2.4,2.2,2.8,3.1,2.9,3.4,3.8,3.6,4.1,4.4,4.8]} labels={['D1','','','','D15','','','','D30','','','D60']} height={130}/>
            <div style={{ background: '#fff', border: `1px solid ${Q.line}`, borderRadius: 10, padding: 14 }}>
              <div style={{ fontFamily: 'Chakra Petch', fontWeight: 600, fontSize: 13, color: Q.ink, marginBottom: 10 }}>{tx('Top 5 SKUs por margem líquida real')}</div>
              <QlikHBars
                rows={[
                  { label: tx('Ração Premium 15kg'), v: 34.2, display: '34,2%' },
                  { label: tx('Suplemento Línea'),   v: 28.7, display: '28,7%' },
                  { label: tx('Acessório Pet Pro'),  v: 24.1, display: '24,1%' },
                  { label: tx('Farmaco Vet Linha'),  v: 18.9, display: '18,9%' },
                  { label: tx('Higiene & Limpeza'),  v: 14.2, display: '14,2%' },
                ]}
                max={40}
                color={Q.turq2}
              />
  ...
  return (
    <QlikFrame title={tx('GESTÃO ESTRATÉGICA · VISÃO 360° · C-LEVEL')} subtitle={tx('Consolidado · 7 filiais · MTD')} tabs={[tx('Visão 360°'), tx('Comparativo YoY'), tx('Simulação'), 'Benchmark']} activeTab={0}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 10 }}>
        ...
        <QlikKPI compact label={tx('Ciclo de Caixa')} value="42 dias" delta="-6 dias" trend="up" color={Q.pos} spark={<Sparkline data={[52,50,48,47,45,43,42]} color={Q.pos} fill="rgba(46,139,87,0.15)"/>}/>
        <QlikKPI compact label={tx('OTIF Global')} value="94,2%" delta="+1,8pp" trend="up" color={Q.navy} spark={<Sparkline data={[89,90,91,92,92.8,93.5,94.2]}/>}/>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.25fr 0.85fr', gap: 10, marginBottom: 10 }}>
        <QlikHBars title={tx('Performance por filial')} rows={[
        ...
        <div style={{ background: '#fff', border: `1px solid ${Q.line}`, borderRadius: 10, padding: 14, display: 'flex', flexDirection: 'column' }}>
          <div style={{ fontFamily: 'Chakra Petch', fontWeight: 600, fontSize: 13, color: Q.ink, marginBottom: 10 }}>{tx('Saúde do negócio')}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1, justifyContent: 'space-evenly' }}>
            {[
              { v: '84%', l: tx('Positivação'), s: 'Meta 80%', color: Q.turq2, data: [76,78,79,80,81,83,84] },
              { v: '94%', l: 'OTIF', s: 'Meta 90%', color: Q.navy, data: [88,89,90,91,92,93,94] },
              { v: '91%', l: tx('Assertividade IA'), s: 'Meta 85%', color: Q.pos, data: [84,85,87,88,89,90,91] },
            ].map((k, i) => (
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${Q.line}`, borderRadius: 10, padding: 14, display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 6 }}>
        <div>
          <div style={{ fontFamily: 'Chakra Petch', fontWeight: 600, fontSize: 13, color: Q.ink }}>{tx('Receita · evolução 12m')}</div>
          <div style={{ fontSize: 11, color: Q.muted, marginTop: 2 }}>YoY +28%</div>
        </div>
        <div style={{ fontSize: 9.5, fontWeight: 600, color: Q.navy, background: 'rgba(117,227,228,0.22)', border: `1px solid ${Q.turq2}`, borderRadius: 99, padding: '3px 9px', letterSpacing: '0.06em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{tx('Projeção IA')}</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 14, fontSize: 10.5, color: Q.muted, marginTop: 4, flexWrap: 'wrap' }}>
        <span><span style={{ color: Q.pos, fontWeight: 700 }}>●</span> {tx('Melhor mês: R$ 19,8M (Mai/25)')}</span>
        <span><span style={{ color: Q.neg, fontWeight: 700 }}>●</span> {tx('Pior mês: R$ 14,2M (Jan/25)')}</span>
        <span><span style={{ color: Q.navy, fontWeight: 700 }}>┄</span> {tx('Projeção IA · próx. 2 meses')}</span>
      </div>
    </div>
  );
}
